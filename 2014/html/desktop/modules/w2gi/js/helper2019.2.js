	...
	updateRecords : function (recs) {
		...
		recs.each( function (e) {
			...
			if (e.NAME) {
				...
				e.PROVINCE_URLENC = e.PROVINCE.toLowerCase();
			}

			if (e.HAPPYHOURMENU || e.CLIENTKEY == '68'){
				e.HAPPYHOURFLAG = '1';
				if(e.CLIENTKEY == '68'){
...
function loadSlippyMap () {
	...
	// var respstate = null;
	// document.observe('w2gi:proximityresponse', function checkstate(response) {
	// 	respCity = response.memo.geoipData.CITY;
	// 	respState = response.memo.geoipData.STATE;
	// 	respZip = response.memo.geoipData.POSTALCODE;

	// 		console.log(respCity + ', ' + respState + ', ' +respZip); 	
	// });
	$('fToggle').observe('click',function(e){
		setTimeout(function(){
			var lCity = ace.table.data.panel.options.attributes.CITY;
			var lState = ace.table.data.panel.options.attributes.STATE;
			var lZip = ace.table.data.panel.options.attributes.POSTALCODE;
			 if (lCity == ''){
			 	lCity = ' ';
			 }
			 if (lState == ''){
			...
			var locationInfo = lCity + ',' + lState + ',' + lZip;
			var locationInfo2 = locationInfo.replace(new RegExp(' ,','g'), '');
			var locationInfo3 = locationInfo2.replace(/,\s*$/, "");
			console.log(locationInfo3);
			FLOOD2('ccf', 'ccflocsf', locationInfo3);		
		}, 100);
	});
}
...
function moreInfo(event) {
	var moreinfosec = event.target.parentNode.parentNode.firstElementChild;
	moreinfosec.click();
}
...
function toggleRes(event) {
	console.log('hit');
	var taborg = document.getElementById('default-org');
	setTimeout(function(){
		taborg.click();
	}, 100);
}
